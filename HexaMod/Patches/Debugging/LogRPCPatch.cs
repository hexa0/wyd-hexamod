...
namespace HexaMod.Patches.Debugging
{
	...
	internal class RPCLog
	{
		...
		static void RPCPhotonTargetsLogPatch(PhotonView view, string methodName, PhotonTargets target, bool encrypt, params object[] parameters)
		{
			...
			Mod.Debug($"RPC from \"{view.name}\" sent to PhotonTargets \"{target}\" invoking \"{methodName}\" with params: {parameters}");
		}
		...
		{
			Mod.Debug($"RPC from \"{view.name}\" sent to PhotonPlayer \"{targetPlayer}\" invoking \"{methodName}\" with params: {parameters}");
		}
	}
}
