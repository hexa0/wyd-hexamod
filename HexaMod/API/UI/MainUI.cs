...
using HexaMod.API.UI.Element.Control.TextInputField;
...
namespace HexaMod.API.UI
{
	public class MainUI : MonoBehaviour
	{
		...
		internal static class ButtonCallbacks
		{
			...
			{
				PlayerPrefs.SetString("HMV2_BCharacterModel", modelName);
			}
			...
			{
				PlayerPrefs.SetString("HMV2_DCharacterModel", modelName);
			}

			public static void SaveDadShirt(string materialName)
			{
				PlayerPrefs.SetString("HMV2_DShirtMaterial", materialName);
			}
		...
		public void Init()
		{
			...
			{ // Player Customization Menu
				...
				dadPlayerController.enabled = false;
				dadPlayerController.characterController.stepOffset = 0f;

				dadModelSwapper.initModel = PlayerPrefs.GetString("HMV2_DCharacterModel", "default");
				dadModelSwapper.initShirt = PlayerPrefs.GetString("HMV2_DShirtMaterial", "default");
				dadModelSwapper.initShirtColor = new Color().FromHex(GetCurrentDadShirtColorHex());
				dadModelSwapper.initSkinColor = new Color().FromHex(GetCurrentDadSkinColorHex());
				dadModelSwapper.SetDadModel();
				dadModelSwapper.Init();
				babyModelSwapper = characterPreviewCanvas.Find("Baby001").AddComponent<CharacterModelSwapper>();
				HexaBabyController babyPlayerController = babyModelSwapper.gameObject.AddComponent<HexaBabyController>();
				babyPlayerController.enabled = false;
				babyPlayerController.characterController.stepOffset = 0f;
				babyModelSwapper.initModel = PlayerPrefs.GetString("HMV2_BCharacterModel", "default");
				babyModelSwapper.initSkinColor = new Color().FromHex(GetCurrentBabySkinColorHex());
				babyModelSwapper.SetBabyModel();
				babyModelSwapper.Init();
				...
				dadStack.AddChild(new WHexColorInputField()
					...
					{
						PlayerPrefs.SetString("HMV2_DShirtColor", hex);
					})
					.SetText("Shirt Color (Hex)")
					.SetFieldText(GetCurrentDadShirtColorHex()));
				...
				dadStack.AddChild(new WHexColorInputField()
					...
					.AddSubmitListener((color, hex) =>
					{
						PlayerPrefs.SetString("HMV2_DSkinColor", hex);
					})
				...
				babyStack.AddChild(new WHexColorInputField()
					...
					.AddSubmitListener((color, hex) =>
					{
						PlayerPrefs.SetString("HMV2_BSkinColor", hex);
					})
					.SetText("Skin Color (Hex)")
		...
		public static string GetCurrentDadShirtColorHex()
		{
			return PlayerPrefs.GetString("HMV2_DShirtColor", "#E76F3D");
		}

		public static string GetCurrentDadSkinColorHex()
		{
			return PlayerPrefs.GetString("HMV2_DSkinColor", "#CC9485");
		}

		public static string GetCurrentBabySkinColorHex()
		{
			return PlayerPrefs.GetString("HMV2_BSkinColor", "#CDA7A4");
		}

		public static string GetCurrentDadModel()
		{
			return PlayerPrefs.GetString("HMV2_DCharacterModel", "default");
		}
